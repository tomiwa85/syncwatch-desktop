...
class ApiClient {
  ...
  Future<RoomSummaryData> createRoom({
    ...
    final body = <String, dynamic>{'visibility': visibility, 'playbackControl': playbackControl};
    if (password != null && password.isNotEmpty) body['password'] = password;
    final j = await _authed('POST', '/api/rooms', body);
    return RoomSummaryData.fromJson(asMap(j['room']));
  }

  Future<RoomSummaryData> getRoom(String code) async {
    final j = await _authed('GET', '/api/rooms/${code.toUpperCase()}');
    return RoomSummaryData.fromJson(asMap(j['room']));
  }
  ...
  Future<List<RoomSummaryData>> listPublicRooms() async {
    final j = await _authed('GET', '/api/rooms?visibility=public');
    return (j['rooms'] as List).map((r) => RoomSummaryData.fromJson(asMap(r))).toList();
  }

  /// Rooms the current user is hosting.
  Future<List<RoomSummaryData>> listMyRooms() async {
    final j = await _authed('GET', '/api/users/me/rooms');
    return (j['rooms'] as List).map((r) => RoomSummaryData.fromJson(asMap(r))).toList();
  }

  Future<void> deleteRoom(String code) => _authed('DELETE', '/api/rooms/${code.toUpperCase()}');

  Future<void> deleteAccount() => _authed('DELETE', '/api/users/me');

  Future<List<WatchHistoryEntry>> getHistory() async {
    final j = await _authed('GET', '/api/users/me/history');
    return (j['entries'] as List).map((e) => WatchHistoryEntry.fromJson(asMap(e))).toList();
  }

  Future<void> deleteHistoryEntry(String code) =>
      _authed('DELETE', '/api/users/me/history/${code.toUpperCase()}');
}
